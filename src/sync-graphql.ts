...
class GraphQLSyncService {
  ...
  constructor(dataSource: DataSourceConfig) {
    ...
    this.graphqlEndpoint = dataSource.connection.endpoint;
  ...
  async syncAll(targetDbId?: string | any): Promise<void> {
    ...
    try {
      ...
      for (const queryConfig of queries) {
        const progress = new ProgressReporter(queryConfig.name);
        await this.syncEntity(queryConfig, targetDbId, progress);
      }
  ...
  /**
   * Build a count query from the main query (simplified version for total count)
   */
  private buildCountQuery(queryConfig: GraphQLQuery): string {
    // This is a simplified approach - in practice, you might need more
    // sophisticated query modification based on your GraphQL schema
    return queryConfig.query.replace(/first:\s*\$first/, "first: 0");
  }
...
export { GraphQLSyncService, GraphQLQuery, GraphQLHeaders };
