...
export async function requireWorkspace(
  ...
  try {
    ...
    const workspaceId = c.req.header('x-workspace-id') || session.activeWorkspaceId;
    ...
    if (!workspaceId) {
      ...
      await workspaceService.switchWorkspace(user.id, workspaces[0].workspace._id.toString());
    } else {
