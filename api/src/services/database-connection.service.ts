...
export class DatabaseConnectionService {
   ...
   */
  async testConnection(database: IDatabase): Promise<{ success: boolean; error?: string }> {
    try {
      switch (database.type) {
        ...
        default:
          return { success: false, error: `Unsupported database type: ${database.type}` };
      }
    } catch (error) {
      return { 
        success: false, 
  ...
  private async testMongoDBConnection(database: IDatabase): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      if (typeof query === 'string') {
        ...
        switch (query.operation) {
          case 'find':
            result = await collection.find(query.filter || {}, query.options || {}).toArray();
            break;
          case 'findOne':
            result = await collection.findOne(query.filter || {}, query.options || {});
            break;
          case 'aggregate':
            result = await collection.aggregate(query.pipeline || [], query.options || {}).toArray();
            break;
          case 'insertMany':
            result = await collection.insertMany(query.documents || [], query.options || {});
            break;
          case 'updateMany':
            result = await collection.updateMany(query.filter || {}, query.update || {}, query.options || {});
            break;
          case 'deleteMany':
            result = await collection.deleteMany(query.filter || {}, query.options || {});
            break;
          case 'updateOne':
            result = await collection.updateOne(query.filter || {}, query.update || {}, query.options || {});
            break;
          case 'deleteOne':
            result = await collection.deleteOne(query.filter || {}, query.options || {});
            break;
          default:
            return { success: false, error: `Unsupported MongoDB operation: ${query.operation}` };
        }
  ...
  private async executeMongoDBJavaScriptQuery(db: Db, query: string): Promise<any> {
    ...
    switch (operation) {
      ...
      case 'updateMany':
        return await collection.updateMany(args[0] || {}, args[1] || {}, args[2] || {});
      case 'deleteMany':
  ...
  private async testPostgreSQLConnection(database: IDatabase): Promise<{ success: boolean; error?: string }> {
  ...
  private async createPostgreSQLConnection(database: IDatabase): Promise<PgClient> {
  ...
  private async executePostgreSQLQuery(database: IDatabase, query: string): Promise<QueryResult> {
  ...
  private async testMySQLConnection(database: IDatabase): Promise<{ success: boolean; error?: string }> {
  ...
  private async createMySQLConnection(database: IDatabase): Promise<mysql.Connection> {
  ...
  private async executeMySQLQuery(database: IDatabase, query: string): Promise<QueryResult> {
  ...
  private async testSQLiteConnection(database: IDatabase): Promise<{ success: boolean; error?: string }> {
    try {
      const db = await open({
  ...
  private async executeSQLiteQuery(database: IDatabase, query: string): Promise<QueryResult> {
  ...
  private async testMSSQLConnection(database: IDatabase): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'MSSQL connection failed' 
      };
    } finally {
      if (pool) {
        await pool.close();
  ...
  private async createMSSQLConnection(database: IDatabase): Promise<ConnectionPool> {
  ...
  private async executeMSSQLQuery(database: IDatabase, query: string): Promise<QueryResult> {
