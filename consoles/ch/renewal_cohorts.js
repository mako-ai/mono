db.ch_close_leads.aggregate([
  ...
  // 8. Compute totals and renewal rates
  {
    $project: {
      ...
      renewed: {
        $arrayElemAt: [
          { $filter: { input: "$statuses", cond: { $eq: ["$$this.renewal_status", "Renewed"] } } },
          0
        ]
      }
    }
  ...
  // 9. Calculate renewal rate and round to 2 decimals
  {
    $project: {
      ...
      renewal_rate: {
        ...
        "2024-01": { $cond: [ { $eq: ["$total.2024-01", 0] }, null, { $round: [{ $divide: ["$renewed.2024-01", "$total.2024-01"] }, 2] } ] },
        "2024-02": { $cond: [ { $eq: ["$total.2024-02", 0] }, null, { $round: [{ $divide: ["$renewed.2024-02", "$total.2024-02"] }, 2] } ] },
        "2024-03": { $cond: [ { $eq: ["$total.2024-03", 0] }, null, { $round: [{ $divide: ["$renewed.2024-03", "$total.2024-03"] }, 2] } ] },
        "2024-04": { $cond: [ { $eq: ["$total.2024-04", 0] }, null, { $round: [{ $divide: ["$renewed.2024-04", "$total.2024-04"] }, 2] } ] },
        "2024-05": { $cond: [ { $eq: ["$total.2024-05", 0] }, null, { $round: [{ $divide: ["$renewed.2024-05", "$total.2024-05"] }, 2] } ] },
        "2024-06": { $cond: [ { $eq: ["$total.2024-06", 0] }, null, { $round: [{ $divide: ["$renewed.2024-06", "$total.2024-06"] }, 2] } ] },
        "2024-07": { $cond: [ { $eq: ["$total.2024-07", 0] }, null, { $round: [{ $divide: ["$renewed.2024-07", "$total.2024-07"] }, 2] } ] },
        "2024-08": { $cond: [ { $eq: ["$total.2024-08", 0] }, null, { $round: [{ $divide: ["$renewed.2024-08", "$total.2024-08"] }, 2] } ] },
        "2024-09": { $cond: [ { $eq: ["$total.2024-09", 0] }, null, { $round: [{ $divide: ["$renewed.2024-09", "$total.2024-09"] }, 2] } ] },
        "2024-10": { $cond: [ { $eq: ["$total.2024-10", 0] }, null, { $round: [{ $divide: ["$renewed.2024-10", "$total.2024-10"] }, 2] } ] },
        "2024-11": { $cond: [ { $eq: ["$total.2024-11", 0] }, null, { $round: [{ $divide: ["$renewed.2024-11", "$total.2024-11"] }, 2] } ] },
        "2024-12": { $cond: [ { $eq: ["$total.2024-12", 0] }, null, { $round: [{ $divide: ["$renewed.2024-12", "$total.2024-12"] }, 2] } ] }
      }
    }
  },
  // 10. Final output
  { $project: { result: { $concatArrays: ["$statuses", ["$total", "$renewal_rate"]] } } },
  { $unwind: "$result" },
