...
db.es_close_opportunities.aggregate([
  { $match: {
      ...
      $expr: {
        $ne: [
          "$date_won",
          { $getField: { field: "cf_GdzdOESUbPaVk8HHjXJackgVHHpG17oYnSDQqtxo5GB", input: "$custom" } }
        ]
      }
  }},
  { $addFields: {
      month: { $substr: [ "$date_won", 0, 7 ] },
      product: { $getField: { field: "cf_M2IFcDmkpaKL6mIK90Cr7AzGL6i8AvAspixN3AeS1IV", input: "$custom" } }
  }},
  { $group: {
  ...
  { $addFields: {
      result: {
        $mergeObjects: [
          ...
          months.reduce((obj, m) => {
            obj[m] = { $ifNull: [ { $getField: { field: m, input: "$monthlyCounts" } }, 0 ] };
            return obj;
          }, {})
        ]
      }
