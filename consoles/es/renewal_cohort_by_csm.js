...
db.es_close_leads.aggregate([
      ...
      status_category: {
        $switch: {
          branches: [
            {
              case: {
                $regexMatch: { input: "$status_label", regex: /customer/i },
              },
              then: "Customer",
            },
            {
              case: {
                $regexMatch: { input: "$status_label", regex: /churn/i },
              },
              then: "Churn",
            },
          ],
  ...
  /* 5️⃣  aggregate per CSM + wonMonth to compute renewal metrics   */
  {
    $group: {
      ...
      renewed: {
        $sum: { $cond: [{ $eq: ["$status_category", "Customer"] }, 1, 0] },
      },
      total: { $sum: 1 },
    },
  },
  ...
  {
    $replaceRoot: {
      newRoot: { $mergeObjects: [{ csm_owner: "$_id" }, "$months_obj"] },
    },
  },
