...
db.es_close_leads.aggregate([
  /* 1️⃣  Pull the Close custom-fields we need */
  {
    $addFields: {
      ...
      renewal_probability: { $getField: { field: "Renewal Probability", input: "$custom" } },
      client_won_date_raw: { $getField: { field: "Client Won Date",     input: "$custom" } }
    }
  },
  ...
  /* 5️⃣  Derive all RealAdvisor metrics we need */
  {
    $addFields: {
      ...
      appraisal_requests_count: {
        $sum: {
          $map: {
            input: { $ifNull: ["$team.teams_users", []] },
            as: "tu",
            in: { $ifNull: ["$$tu.user.appraisal_requests.aggregate.count", 0] }
          }
        }
      },
      crm_leads_count: {
      ...
      /* ---------- Transaction metrics ---------- */
      transactions_count:        { $ifNull: ["$team.property_transactions_aggregate.aggregate.count", 0] },
      last_transaction_date_raw: "$team.property_transactions_aggregate.aggregate.max.created_at",
    ...
    $addFields: {
      ...
      last_review_date: {
        $cond: [
          ...
          { $dateToString: { format: "%Y-%m-%d", date: "$last_review_date_raw" } },
      ...
      last_transaction_date: {
        $cond: [
          ...
          { $dateToString: { format: "%Y-%m-%d", date: "$last_transaction_date_raw" } },
