  ...
  { $facet: {
    wonStats: [
      ...
      { $addFields: {
        d2c: {
          $divide: [
            { $subtract: [ { $toDate: "$date_won" }, { $toDate: "$date_created" } ] },
            1000 * 60 * 60 * 24
          ]
        }
      }},
    ...
    conv: [
      { $match: {
        $or: [
          { status_type: "won", date_won: { $ne: null }, date_created: { $ne: null } },
          { status_type: "lost" }
        ]
      }},
      { $addFields: {
      ...
      { $group: {
        ...
        lost: { $sum: { $cond: [{ $eq: ["$status_type", "lost"] }, 1, 0] } },
        w_7: { $sum: { $cond: [
          { $and: [ { $eq: ["$status_type", "won"] }, { $lte: ["$d2c", 7] }, { $gte: ["$d2c", 0] }] }, 1, 0
        ]}},
        w_14: { $sum: { $cond: [
          { $and: [ { $eq: ["$status_type", "won"] }, { $lte: ["$d2c", 14] }, { $gte: ["$d2c", 0] }] }, 1, 0
        ]}},
        w_30: { $sum: { $cond: [
          { $and: [ { $eq: ["$status_type", "won"] }, { $lte: ["$d2c", 30] }, { $gte: ["$d2c", 0] }] }, 1, 0
        ]}}
      }},
      { $project: {
        _id: 0,
        ...
        cr: {
          $cond: [
            { $gt: [{ $add: ["$won", "$lost"] }, 0] },
            { $round: [ { $divide: [ "$won", { $add: ["$won", "$lost"] } ] }, 2 ] },
            null
          ]
        },
        cr_7: {
          $cond: [
            { $gt: [{ $add: ["$won", "$lost"] }, 0] },
            { $round: [ { $divide: [ "$w_7", { $add: ["$won", "$lost"] } ] }, 2 ] },
            null
          ]
        },
        cr_14: {
          $cond: [
            { $gt: [{ $add: ["$won", "$lost"] }, 0] },
            { $round: [ { $divide: [ "$w_14", { $add: ["$won", "$lost"] } ] }, 2 ] },
            null
          ]
        },
        cr_30: {
          $cond: [
            { $gt: [{ $add: ["$won", "$lost"] }, 0] },
            { $round: [ { $divide: [ "$w_30", { $add: ["$won", "$lost"] } ] }, 2 ] },
            null
          ]
        }
      }}
            ...
            "$$row",
            {
              cr: {
                $let: {
                  vars: { c: {
                    $arrayElemAt: [
                      { $filter: { input: "$conv", as: "c", cond: { $and: [
              ...
              cr_7:  { $let: { vars: { c: { $arrayElemAt: [ { $filter: { input: "$conv", as: "c", cond: { $and: [ { $eq: ["$$c.country", "$$row.country"] }, { $eq: ["$$c.closer", "$$row.closer"] } ] } } }, 0 ] } }, in: "$$c.cr_7"  } },
              cr_14: { $let: { vars: { c: { $arrayElemAt: [ { $filter: { input: "$conv", as: "c", cond: { $and: [ { $eq: ["$$c.country", "$$row.country"] }, { $eq: ["$$c.closer", "$$row.closer"] } ] } } }, 0 ] } }, in: "$$c.cr_14" } },
              cr_30: { $let: { vars: { c: { $arrayElemAt: [ { $filter: { input: "$conv", as: "c", cond: { $and: [ { $eq: ["$$c.country", "$$row.country"] }, { $eq: ["$$c.closer", "$$row.closer"] } ] } } }, 0 ] } }, in: "$$c.cr_30" } }
            }
          ]
        }
      }
